...
const AddSuppliersTransactionDialog = ({ id }) => {
    ...
    return (
        <dialog id="add_supplier_transaction" className="modal modal-bottom sm:modal-middle">
                ...
                <div className="modal-action flex-wrap justify-center w-full gap-10">
                    <form method="dialog" className='flex flex-col w-full'>
                        ...
                        <div className=" w-full text-left">
                            ...
                            <div className="">
                                <input type='number' className="border-2 w-full sm:text-md mt-2 px-4 py-2" onChange={(e) => { setSupplierPayload({ ...supplierPayload, payment: e.target.value }); console.log(e.target.value) }} />
                                {supplierPayload.payout_date === false && <span className="text-[red]" data-valmsg-for="UserName" data-valmsg-replace="true">Payment amount is required</span>}
                    ...
                    <button className="btn w-[40%]" onClick={() => document.getElementById('add_supplier_transaction').close()}>Close</button>
