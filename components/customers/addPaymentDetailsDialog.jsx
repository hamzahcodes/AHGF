...
const AddPaymentDetailsDialog = ({ id }) => {
    ...
    const [customerPayload, setCustomerPayload] = useState({ payout_date: '', amount: '' });
    ...
    const submitHandler = async () => {
        ...
        if (customerPayload.payout_date === '' || customerPayload.payout_date === false) {
                ...
                <div className="modal-action flex-wrap justify-center w-full gap-10">
                    <form method="dialog" className='flex flex-col w-full'>
                        ...
                        <div className=" w-full text-left">
                            ...
                            <div className="">
                                <input type='date' className="border-2 w-full sm:text-md mt-2 px-4 py-2"   onChange={(e) => { setCustomerPayload({ ...customerPayload, payout_date: e.target.value });console.log(e.target.value) }} />
                                {customerPayload.payout_date === false && <span className="text-[red]" data-valmsg-for="UserName" data-valmsg-replace="true">Payout date is required</span>}
                    ...
                    <button className="btn w-[40%]" onClick={() => document.getElementById('my_modal_15').close()}>Close</button>
